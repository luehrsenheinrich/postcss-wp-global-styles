...
/**
 * Generate the stylsheet needed for one color gradient preset.
 *
 * @param {object} element - The element to generate the stylesheet for.
 *
 * @returns {object} The AST representing the stylesheet.
 */
function colorGradientGenerator (element) {
  const stylesheet = [];

  const gradientRule = new Rule({ selector: `.has-${element.slug}-gradient-background` });
  gradientRule.append(new Declaration({ prop: 'background', value: element.gradient }));
  stylesheet.push(gradientRule);
